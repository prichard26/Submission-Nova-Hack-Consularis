...
Step metadata includes: **name**, **actor**, **duration**, **description**, **inputs**, **outputs**, **risks**, **automation potential**, **automation notes**.
...
- All changes are made through **tools** that update the in-memory BPMN model; the API returns updated BPMN XML so the diagram stays in sync.
- If the LLM does not call tools, a **fallback** can still apply simple edits when the message clearly mentions a step (by name or ID) and an action (e.g. duration, rename, remove).

For technical details (tools, store, API), see [GRAPH_STRUCTURE.md](GRAPH_STRUCTURE.md) and [GRAPH_OPERATIONS_VERIFICATION.md](GRAPH_OPERATIONS_VERIFICATION.md).
...
| [GRAPH_STRUCTURE.md](GRAPH_STRUCTURE.md) | BPMN format, model, API contracts. |
| [GRAPH_OPERATIONS_VERIFICATION.md](GRAPH_OPERATIONS_VERIFICATION.md) | Backend operations and agent tools. |
