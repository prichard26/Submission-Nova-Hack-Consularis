...
- **Agent tools:** constrained operations on the graph (`get_node`, `update_node`, `add_edge`, etc.) so Nova produces incremental, validated updates instead of raw XML.
- **Separation:** the analyzer and report writer only consume metrics and produce text, which kept prompts and behavior clean.
